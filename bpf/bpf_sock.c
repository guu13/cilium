		      ...
		      const bool in_hostns __maybe_unused)
{
	...
	barry_printk("sock4_wildcard_lookup:%u, port:%u, \n", bpf_htonl(key->address), bpf_htons(key->dport));
	...
	return NULL;
wildcard_lookup:
	barry_printk("sock4_wildcard_lookup goto wildcard_lookup\n");
	key->address = 0;
			   ...
			   const bool in_hostns __maybe_unused)
{
	...
	if(svc)
		barry_printk("sock4_wildcard_lookup_full nodeport lb4_svc_is_nodeport\n");
	...
	if (!svc) {
		...
		if(svc)
			barry_printk("sock4_wildcard_lookup_full hostport \n");
	}
#endif /* ENABLE_NODEPORT */
	return svc;
}
					     ...
					     const bool udp_only)
{
	...
	//barry_printk("__sock4_xlate_fwd %u, %u, %u \n", svc-count, svc-flag, key.address);
...
{
	barry_printk("cgroup/connect4 ip:%u, port:%u\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
	if (sock_is_health_check(ctx))
		return __sock4_health_fwd(ctx);
	barry_printk("cgroup/connect4 ip:%u, port:%u, __sock4_health_fwd\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
	__sock4_xlate_fwd(ctx, ctx, false);
	barry_printk("cgroup/connect4 ip:%u, port:%u , __sock4_xlate_fwd\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
	return SYS_PROCEED;
}
...
{
	barry_printk("cgroup/post_bind4\n");
...
{
	barry_printk("cgroup/post_bind4 ip:%u, port:%u\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
...
{
	barry_printk("cgroup/sendmsg4 ip:%u, port:%u\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
...
{
	barry_printk("cgroup/recvmsg4 ip:%u, port:%u\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
...
{
	barry_printk("cgroup/getpeername4 ip:%u, port:%u\n", bpf_htonl(ctx->user_ip4), bpf_htons(ctx->user_port));
